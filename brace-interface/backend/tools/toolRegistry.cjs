...
const { createGitNexusTools } = require("./gitnexusTools.cjs");
const voiceboxProvider = require("../voice/voiceboxProvider.cjs");
...
function createToolRegistry(deps) {
  const gitnexusTools = createGitNexusTools();
  ...
  return [
    ...
    ...gitnexusTools.map((t) => tool(t)),
    // Voicebox voice tools
    tool({ name: "voicebox.status", description: "Check Voicebox voice server health.", riskLevel: "low", requiredPermission: "systemInfo", execute: () => voiceboxProvider.healthCheck() }),
    tool({ name: "voicebox.speak", description: "Speak text using Voicebox TTS.", riskLevel: "low", requiredPermission: "systemInfo", execute: ({ text, profile }) => voiceboxProvider.speak(text, { profile }) }),
    tool({ name: "voicebox.profiles", description: "List available Voicebox voice profiles.", riskLevel: "low", requiredPermission: "systemInfo", execute: () => voiceboxProvider.listProfiles() }),
  ];
}
