...
import { defaultVoiceConfig } from "../voice/voiceStateStore";
  ...
  return (
    <div className="mx-auto grid max-w-7xl gap-6 xl:grid-cols-[1fr_420px]">
      <div className="space-y-6">
        <section className="glass-panel rounded-2xl border border-white/10 p-6 text-left">
          <p className="text-xs uppercase tracking-[0.24em] text-cyan-200 font-mono">Voice engine</p>
        ...
        <section className="grid gap-4 md:grid-cols-2 text-left">
          <Panel title="Voice mode">
            <Select 
              label="Voice Provider" 
              value={config.voiceProvider || "voicebox"} 
              onChange={(voiceProvider) => onUpdate({ voiceProvider })} 
              options={[
                ["voicebox", "Local Voicebox (Whisper + TTS)"],
                ["browser-fallback", "Browser fallback (Web Speech API)"]
              ]} 
            />
          ...
          <Panel title="Safety controls">
            ...
            <button className="secondary-button mt-4" onClick={() => onUpdate(defaultVoiceConfig)} type="button"><RotateCcw size={16} /> Reset voice</button>
          </Panel>
        </section>
      </div>
      ...
      <aside className="space-y-6 text-left">
        <Panel title="Provider status">
          <StatusLine label="STT Active" value={status?.sttProvider ?? "checking"} />
          <StatusLine label="TTS Active" value={status?.ttsProvider ?? "checking"} />
          <StatusLine label="VAD Engine" value={status?.vadProvider ?? "checking"} />
          <StatusLine label="Connection" value={status?.voiceboxConnected ? "Voicebox Connected ✓" : "Voicebox Offline (Fallback active)"} />
          <button className="secondary-button w-full mt-2" onClick={onRefresh} type="button"><RefreshCw size={16} /> Refresh Status</button>
        </Panel>
        
        <Panel title="Voicebox Details">
          <div className="text-xs space-y-2 text-slate-400">
            <p><strong>Configured Endpoint:</strong> {config.voiceboxBaseUrl}</p>
            <p><strong>Default Profile:</strong> {config.voiceboxDefaultProfile || "None (uses default)"}</p>
          </div>
          {(status?.setup ?? []).slice(0, 5).map((line) => <code className="block rounded-xl bg-black/30 p-3 text-[11px] font-mono text-cyan-200 mt-2 whitespace-pre-wrap leading-relaxed" key={line}>{line}</code>)}
          {status?.setup?.length === 0 && <p className="text-xs text-emerald-200">All voice engine components are fully verified.</p>}
        </Panel>

        <Panel title="Live transcript preview">
          <p className="min-h-16 rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-slate-300 leading-relaxed font-mono">{partialTranscript || transcript || "Waiting for audio input..."}</p>
          {error && <p className="rounded-xl border border-rose-500/25 bg-rose-500/10 p-3 text-sm text-rose-100 font-mono mt-2">{error}</p>}
        </Panel>
      </aside>
    </div>
  );
...
function Toggle({ checked, label, onClick }: { checked: boolean; label: string; onClick: () => void }) {
  return (
    <button className="flex w-full items-center justify-between rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-sm text-slate-300 hover:border-white/20 transition" onClick={onClick} type="button">
      {label}
      <span className={["toggle-pill scale-75", checked ? "toggle-pill-on" : ""].join(" ")}><span /></span>
    </button>
  );
}
