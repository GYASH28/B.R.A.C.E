...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".brace_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".brace_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using B.R.A.C.E. profile")
                brace = str(Path.home() / ".brace_profiles" / "firefox_brace")
                Path(brace).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(brace, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".brace_profiles" / "safari")
        ...
        brace_profile = str(Path.home() / ".brace_profiles" / self.browser_name)
        Path(brace_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with B.R.A.C.E. profile: {brace_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(brace_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with B.R.A.C.E. profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "brace_screenshot.png")
