...
_SYSTEM_PROMPT = (
    "You are B.R.A.C.E., an advanced AI assistant. "
...
def _probe_camera(index: int, backend: int, warmup: int = 5) -> bool:
    ...
    if not ret or frame is None:
        return False
    return bool(_NP and np.mean(frame) > 8)
...
def _get_camera_index() -> int:
    camera_index = get_runtime_value("camera_index")
...
class _VisionSession:
    ...
    async def _session_loop(self) -> None:
        ...
        if not _GENAI:
            print("[Vision] \u274c google-genai not installed. Cannot start vision session.")
            return
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            self._player.write_log(f"B.R.A.C.E.: {full}")
                            print(f"[Vision] \ud83d\udcac {full}")
                    transcript = []
    ...
    async def _play_loop(self) -> None:
        if not _SD:
            print("[Vision] \u26a0\ufe0f sounddevice not available \u2014 audio playback disabled")
            # Drain the audio queue silently so recv_loop doesn't block
            while True:
